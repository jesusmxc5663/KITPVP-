...
use duel\command\subcommand\JoinSubCommand;
...
class Arena implements Listener, ArenaStats
{
	...
	{
		if (count($this->players) >= 2 || $this->stat == self::RUN || $this->inArena($player))
		     return;
		...
		foreach ($this->players as $target) {
		     $target->sendMessage('§r§b[Duel]: §r§b'.$player->getName().' entrou na partida.');
	    }
	...
	public function quit(Player $player, $msg = false, $quit = false)
	{
		...
		if ($msg) {
			 foreach ($this->players as $target) {
			     $target->sendMessage('§r§b[Duel]: §r§c'.$player->getName().' saiu da partida.');
		     }
		}
	}
	...
	public function dead(Player $player)
	{
		...
		foreach ($this->loader->getServer()->getOnlinePlayers() as $players) {
			$players->sendMessage('§r§b[Duel]: §r§bO jogador §r§f'.$winner->getName().' §r§bvenceu o duelo contra §r§f'.$player->getName().'.');
		}
	}
	...
	public function tick()
	{
		switch ($this->stat) {
			...
			if (count($this->players) >= 2) {
				...
				foreach ($this->players as $player) {
					$player->sendTip('§r§cAguardando por mais 1 jogador...');
				}
				if ($this->time != 5) {
					 $this->time = 5;
				}
	...
	public function startGame()
	{
		...
		$first->teleport(new Position(...$this->data['pos1']));
		
		/** @var Player */
		$second = $this->players[1];
		$second->teleport(new Position(...$this->data['pos2']));
		
		foreach ($this->players as $player) {
			$player->setNameTag('§r§c'.$player->getName()."\n§r§f".floor($player->getHealth()).' §r§c❤');
	...
	{
		foreach ($this->players as $player) {
			$player->sendMessage('§r§b[Duel]: §r§cAcabou o tempo, não houve um vencedor!');
			JoinSubCommand::refreshParticles();
			JoinSubCommand::checkPlayers();
    ...
    /**
      * @param PlayerComm&&PreprocessEvent $event
      */
    public function onCommandPreprocess(PlayerCommandPreprocessEvent $event)
    {
        ...
        if ($this->inArena($player) && $message{0} == '/') {
        	 ...
        	 if ($command[0] == '/sair') {
        	      ...
        	      $player->sendMessage('§r§b[Duel]: §r§cPara sair da partida utilize: /sair.');
                  return;
        	}
        }
    }
